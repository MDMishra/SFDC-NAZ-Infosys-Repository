trigger AB_UpdateMission on US_Account_Target_Program__c (before insert,before update) 
{
    ...
    {
        if(accProgram.Brand_Category_Text__c != '')
            brandCategory.add(accProgram.Brand_Category_Text__c);
    ...
    if(trigger.isInsert)
    {
        for(US_Account_Target_Program__c accProgram:trigger.new)
        {
            if(accProgram.Brand_Category_Text__c !='')
            {
                ...
                {
                    accProgram.Missions_id__c=mapMissionId.get(accProgram.Brand_Category_Text__c);
     ...
     if(trigger.isUpdate)
     {
         ...
         for(US_Account_Target_Program__c accProgram:trigger.new)
         {
            ...
            if(accProgram.Brand_Category_Text__c !='' && accProgram.Brand_Category_Text__c!=oldAcc.Brand_Category_Text__c)
            {
                accProgram.Missions_id__c=mapMissionId.get(accProgram.Brand_Category_Text__c);
            }
         }
